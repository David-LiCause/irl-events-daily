...
1. The **Airtable OAuth connector** — used by this live session/routine to read/write `Sources` records via MCP tool calls.
...
- Everything after that — reading/writing `Sources` rows — is fully automatic via the OAuth connector.

## Idempotency caveat
`scripts/setup_airtable.py` always creates a *new* base — it has no check for an existing `events-daily` base/table. Before running it, check (via the Airtable connector's `list_bases`) whether a base with that table already exists, to avoid creating duplicates.
